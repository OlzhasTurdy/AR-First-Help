...
#pragma warning restore IDE0025
    ...
    {
      // Увеличено значение таймаута для медленных виртуальных камер (было 2000)
      const int timeoutFrame = 10000;
      var count = 0;
      Debug.Log($"Waiting for WebCamTexture ({sourceName}) to start...");
      
      // Ждем, пока камера начнет играть И ширина станет больше 16 (признак инициализации кадра)
      yield return new WaitUntil(() => count++ > timeoutFrame || (webCamTexture.isPlaying && webCamTexture.width > 16));

      if (webCamTexture.width <= 16)
      {
        Debug.LogError($"WebCam timeout reached. Current state: isPlaying={webCamTexture.isPlaying}, size={webCamTexture.width}x{webCamTexture.height}");
        throw new TimeoutException("Failed to start WebCam (Timeout)");
      }
